...
class Batch(ABC):
    ...
    def for_individual_learners(self) -> "Batch":
        ...
        if self.importance_sampling_weights is not None:
            self.importance_sampling_weights = self.importance_sampling_weights.repeat_interleave(self.n_agents).view(
                *self.importance_sampling_weights.shape, self.n_agents
            )
        return self
